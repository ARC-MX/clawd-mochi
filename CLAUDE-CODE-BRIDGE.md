...
| `GET /state` | `{view,busy,term,bl,blv,bg,states,name,pass}` — `bg` 是当前宠物背景色，`states` 是当前主题提供的全部状态（空格分隔），`name`/`pass` 是生效中的设备名与热点密码 |
| `GET /backlight?on=0\|1` · `?v=<0..100>` | 背光开关 / 亮度百分比（存 NVS） |
...
| `POST /theme/commit` | 删除新 manifest 未引用的文件，重启播放器 |

> 注：服务端 `max_uri_handlers = 20`，目前已注册 14 个，新增路由要注意余量，且**必须注册在通配路由之前**（当前没有通配路由，安全）。

### 主题替换为什么是三步

分区只有 2.44 MB，而**一组主题就要 2 MB 左右** —— 两组无法同时在盘上。所以"先暂存新主题、再原子替换"这条常规路径在这里走不通：没有空间放第二份。

于是流程改成 **begin（清空）→ put（逐文件）→ commit（清理 + 重启）**：

- 旧动画先被删掉，**新 manifest 最后写**，这样在新包完整落地之前，旧的状态映射仍然有效
- 上传期间设备上没有主题，播放器已被 `begin` 停止，不会刷错误日志
- 上传中途失败会留下一个残缺主题 —— 这是这套设计**唯一无法排除**的故障模式，网页会明确报出来而不是假装成功

写入是 2 KB 分块流式落盘，2 MB 的包不会占 RAM。文件名来自网络且会拼进路径，含分隔符或 `..` 的一律拒绝。
...
> **状态：已实现并验证。** 固件在 `main/ble_cli.c`（NimBLE 外设），宿主脚本 `tools/mochi_ble.py`。
>
> 设备以 `clawd-mochi` 广播，暴露一个可写特征值：
> - 服务 `0000abf0-0000-1000-8000-00805f9b34fb`
> - 特征 `0000abf1-0000-1000-8000-00805f9b34fb`（write / write-no-response）
>
> 写入的命令与 USB 串口是**同一套**（`w/s/d/a/q`、`status…`、`bg…` 等），
> 内部投递到同一个命令队列、由同一个工作线程执行，见 §方案 E。
> 例外：`img` 图像传输是串口专属（需要连续读取 UART 的原始字节流）。
...
**Claude Code hooks 示例**（与串口版命令完全相同，只把传输换成 BLE）：
...
> ⚠️ 每次调用都要**重新扫描并建立 BLE 连接**（约 1–2 秒），所以 `timeout` 要比串口版给得更宽。频繁触发时 BLE 的连接开销明显高于串口——若设备本来就在 USB 上，优先用方案 E。
...
| `bright <0-100>` | 背光亮度百分比（存 NVS，重启保留）；无参数回显当前档位 |
...
**D（蓝牙）与 E（USB 串口）都已实现并验证**。选择依据很简单：

- **设备插着 USB** → 用 **E**（更快、更省电、零连接开销，还能传图）。
- **设备需要脱离 USB 摆放** → 用 **D**（无需路由器、无需配对流程，代价是每次命令约 1–2 秒的连接开销）。

A（WiFi STA）仍可作为补充——若希望**局域网内多台机器**都能驱动设备（例如手机浏览器直接控制），它有独立价值。B/C 仅在特殊网络环境下才有意义。
